<?php
/** 
 * this is the script that does all the neccessary logic to register a user;
...
if (isset($_POST["submit"])) {
    ...
    $mysqli = new mysqli("localhost", "taiwo", "Nana23", "today");
...
function userQuery($email, $password) {
	...
	$resultresult = mysql_query($queryquery);

	if($resultresult) {

		if(mysql_num_rows($resultresult) > 0) {
			session_regenerate_id();
			$member = mysql_fetch_assoc($resultresult);
			
			$_SESSION['SESSION_ID'] = $member['id'];

			session_write_close();
			header("Location: ../bookavehicle.php");
			//exit();
		}
...
function insertUser( $email, $password1, $password2) {
    ...
    if ($stmt = $mysqli->prepare($sql_insert)) {
        ...
        if (!$stmt->execute()) {
            ...
            //should be deleted this is just to test or show success
            echo "\n insert completed";
        }
       ...
       return $inserted_user_id = $mysqli->insert_id;
